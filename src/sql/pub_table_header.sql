...
CREATE TABLE IF NOT EXISTS `pub_header_option` (
  ...
  `is_required` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '是否必选',
  `is_default` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '是否默认',
  `is_enable` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '是否开启',
  `is_sortable` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '是否可排序',
  PRIMARY KEY (`id`),
